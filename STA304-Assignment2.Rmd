...
We also estimated the proportion of liberal voters in each group divided by a predictor ($\hat{p}^{PS}$) using the mathematical formula. For instance, we estimated the proportion of liberal voters in each age group, and the proportion of liberal voters in each gender group and so on. We were able to check if the 
...
To model the probability of voting for different parties, a multilevel logistic regression model was employed each time. This model incorporated various predictors such as age, gender, marital status and so on. The model was then applied to the survey data, and the results were used to estimate the probability of
